...
impl<L: Language + Default + Common> Generator<L> {
    ...
    pub fn push_def(&mut self, name: String, which: &impl Definition<L>) {
        which.gen_def(&mut self.state, name, &mut self.buffer);
    }

    ///
    pub fn push_write_impl(&mut self, name: String, which: &impl WriteImpl<L>) {
        which.gen_write_impl(&mut self.state, name, &mut self.buffer);
    }

    pub fn push_read_impl(&mut self, name: String, which: &impl ReadImpl<L>) {
...
pub trait WriteImpl<K: Language> {
    fn gen_write_impl(&self, state: &mut K, name: String, out: &mut String);
}

pub trait ReadImpl<K: Language> {
    fn gen_read_impl(&self, state: &mut K, name: String, out: &mut String);
}

pub trait Definition<K: Language> {
    fn gen_def(&self, state: &mut K, name: String, out: &mut String);
}
